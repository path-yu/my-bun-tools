...
(defun clean_final_logic (str / s pos1 pos2 result i len char code) 
  ...
  (while (<= i len) 
    ...
    (if (or (and (>= code 48) (<= code 57)) (and (>= code 65) (<= code 90))
            (and (>= code 97) (<= code 122)) (= code 47) (= code 45)
            (= code 46) (= code 95) (= code 126) (= code 32))
...
(defun process_single_box (pmin pmax / ax ay ray y_hits int_pt cur_y_val top_left 
  ...
  (setq ax (car pmax) ay (cadr pmin) y_hits '())
  (if (setq ss_local_lines (ssget "C" (list (- ax 10) ay) (list (+ ax 10) (+ ay 1500)) '((0 . "LWPOLYLINE,LINE"))))
    (progn 
      (setq ray (vla-addline (vla-get-modelspace (vla-get-activedocument (vlax-get-acad-object))) 
                             (vlax-3d-point (list ax ay 0)) (vlax-3d-point (list ax (+ ay 1500) 0))))
      (setq k 0)
  ...
  (if (>= (length y_hits) 2) 
    (progn 
      ...
      (setq cur_m "" cur_d "" cur_p "" cur_x 0.0 cur_y_pt 0.0)

      (foreach item txt_data 
        (setq tx (car (car item)) ty (cadr (car item)) tstr (cadr item))
        (if (and (>= tx (car s_min)) (<= tx (car s_max)) (>= ty (cadr s_min)) (<= ty (cadr s_max)))
          (progn 
    ...
    (progn 
      ...
      (foreach item data_list 
        (setq json_str (strcat json_str 
          "{"
          "\"materialCode\":\"" (nth 2 item) "\","   ;; 对应 (nth 2 row) 编码
          "\"drawingNumber\":\"" (nth 0 item) "\","  ;; 对应 (nth 0 row) 图号
          "\"fileName\":\"" dwg_name "\","           ;; 补全文件名
          "\"remarks\":\"" (nth 1 item) "\","        ;; 将“图名”存入备注
          "\"x\":\"" (nth 3 item) "\","
          "\"y\":\"" (nth 4 item) "\""
          "}"
        ))
...
;; 保持原有的 EKC 命令也改为上传逻辑 (可选)
(defun c:EKC ()
  (princ "\n全图扫描模式...")
  ;; 这里可以调用类似于 BEXK 但使用 ssget "X" 的逻辑
  ;; 建议直接使用 BEXK 更加精准
  (c:BEXK) 
)
;; ==========================================================
...
;; ==========================================================
(defun c:GTA (/ ss_all i ed vlist p_min p_max area res unique_list ss_txt final_str row item_str)
  (setvar "CMDECHO" 0)
  (setq final_list '() txt_data '() unique_list '())
    ...
    (progn
      ...
      (if (> (length final_list) 0)
        (progn
          ...
          (princ (strcat "\n[预览]: " (if (> (strlen final_str) 50) (strcat (substr final_str 1 50) "...") final_str)))
        )
        (princ "\n[提示] 未找到有效的图框数据。")
      )
    )
