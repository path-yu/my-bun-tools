...
(defun c:P2R (/ p1 p2 p3 p4 ss pt1 pt2)
  (vl-load-com)
  ...
  (if (and p1 p2)
    (progn
      ;; 统一转换为 2D 坐标 (去除 Z 轴)，防止 entmake 或 ssget 识别出错
      (setq pt1 (list (car p1) (cadr p1))
            pt2 (list (car p2) (cadr p2))
            p3  (list (car pt2) (cadr pt1))
            p4  (list (car pt1) (cadr pt2)))

      ;; 2. 创建闭合多段线 (LWPOLYLINE)
      (entmake
        (list
          '(0 . "LWPOLYLINE")
          '(100 . "AcDbEntity")
      ...
      ;; 3. 自动清理：删除这两点范围内原有的散线
      ;; 使用 (command-s) 或检查系统变量防止 undefined 错误
      (if (setq ss (ssget "C" pt1 pt2 '((0 . "LINE"))))
        (progn
          ;; 使用较旧但兼容性更强的 command 写法
          (vl-cmdf "_.erase" ss "")
          (princ (strcat "\n[清理] 已自动删除范围内 " (itoa (sslength ss)) " 条直线。"))
        )
      )
